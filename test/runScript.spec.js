...
${linteErr.stderr}`)
    ...
    expect(taskPromise).toBeAPromise()
      ...
      [
        '-o file1.js.test',
                '-o file2.js.test',
        '--option',
        '--ext=js paraminthemiddle --file=file1.js',
        '--ext=js paraminthemiddle --file=file2.js',
                '--paraminbetween',
                '--dir=. --out=file1.tar.gz',
        '--dir=. --out=file2.tar.gz',
                '--end'
            ],
      {}
        ])
    ...
    it('should throw error for absent command', async () => {
      ...
      const taskPromise = res[0].task()
        ...
        `Command could not be found. You're package.json is probably wrong.`
      )
        }
  })
